...
export function fakeAPIFetch(options) {
  ...
  } else if (options.url.includes("cars")) {
    return handleFakeRequestForCars(options.url);
...
function handleFakeRequestForCars(url) {
  if (url.includes("page")) {
    return handleListRequest(url, cars, modifyCar);
  } else {
    return handleIndividualRequest(url, cars, modifyCar);
  }
}
...
function modifyCar(car) {
  const films = getFilmsThatMatchId(car.id, "cars");
  const residents = getPeopleThatMatchPlanet(car.id);
  return {
    ...car,
    films,
    residents,
